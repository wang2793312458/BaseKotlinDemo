...
import com.example.basekotlindemo.utils.md5
...
class HttpInterceptor : Interceptor {
    ...
    override fun intercept(chain: Chain?): Response {
        ...
        buffer.flush()
        buffer.close()
        nUrl?.addQueryParameter("sign", parameter.md5())?.addQueryParameter("token", token)
        val requestBuilder = request?.newBuilder()?.method(request.method(), request.body())?.url(
              nUrl?.build())
