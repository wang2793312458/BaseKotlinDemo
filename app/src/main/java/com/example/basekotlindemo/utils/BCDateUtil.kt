...
object BCDateUtil {
    ...
    val AM = "AM"
    val PM = "PM"
    val TIME_SECOND = 1000
    val TIME_MINUTE = 60000
    val TIME_HOUR = 3600000
    val TIME_DAY = 86400000
    // 格式：年－月－日 小时：分钟：秒
    // 格式：年－月－日
     ...
     */
    @SuppressLint("SimpleDateFormat")
    fun xiangdeng(date1: String, date2: String): Boolean {
     ...
     * @param format 转换类型
     */
    fun stringToDate(dateStr: String, format: String = dateFormatYMDHMS): Date? {
     ...
     * 日期转字符串
     */
    fun dateToString(date: Date, format: String = dateFormatYMDHMS): String? {
    ...
    fun yearDiff(before: String, after: String): Int {
        val beforeDay = stringToDate(before, dateFormatYMD)
        val afterDay = stringToDate(after, dateFormatYMD)
     ...
     * 获取一天的开始时间
     */
    fun getFristDayTime(dateFormat: String = dateFormatYMDHMS, setTime: String): String {
     ...
     * 获取一天的结束时间
     */
    fun getLastDayTime(dateFormat: String = dateFormatYMDHMS, setTime: String): String {
        val format = SimpleDateFormat(dateFormat, Locale.US)
        val c1 = GregorianCalendar()
        try {
            c1.time = format.parse(setTime)
